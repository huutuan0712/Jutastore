    ...
    public function add(Request $request)
    {
        ...
        if($product_check){
            $verified_purchase = Order::where('orders.user_id',Auth::id())
                                ->join('orders_items','order_id','orders_items.order_id')
                                -> where('orders_items.prod_id',$product_id)->get();
            if($verified_purchase){
                $existsing_rating = Rating::where('user_id',Auth::id())->where('prod_id', $product_id)->exists();
                if($existsing_rating){
            ...
            return redirect()->back()->with('status','The link you followed was borken');
        }
